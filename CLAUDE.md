...
3. `node scripts/sync-shared.js` — copies `plugins/shared/` config + the umbrella `plugins/shared/skills/nio/*` into each plugin's `skills/nio/`, and the focused `plugins/shared/skills/nio-*/` into Claude Code + Codex only.
