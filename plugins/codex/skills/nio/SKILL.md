...
**Scope of capture, in short.** Arming a session sends to `collector.endpoint`: the user prompt that opened each turn, the tool calls it made, token usage, guard verdicts, and audit records. On **Hermes and OpenClaw alike** it also sends conversation content — one `chat` span per LLM call, plus the model's reasoning, its reply, tool arguments and tool output — each scanned for secret-shaped strings and size-capped, with small bodies riding the span and large ones going to the logs signal. One difference remains on **OpenClaw**: its tool spans carry the arguments *and* the result as `gen_ai.tool.call.arguments` / `gen_ai.tool.call.result`, capped at 2048 characters and redacted by JSON key name only — so a secret inside a command string or in tool output is removed from the log record but **still rides that span attribute**. Hermes never puts a tool result on a span at all. While a session is unarmed, none of it leaves the machine.
