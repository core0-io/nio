...
| 3 | Gate | Resolved `{server, tool}` candidates feed the existing `permitted_tools.mcp` / `blocked_tools.mcp` allowlist; audit-only hits inform the audit log without contributing to deny |
