 ...
 * is longer still. Measured on this machine against RFC 5737 TEST-NET-1
 ...
 *
 * A merely *refused* endpoint (RST from the kernel) is far milder —
 * 10–21s, see hermes-exit.test.ts — which is why drop, not refusal, is
 * the case this module is sized against.
 *
 * Losing a span/metric against an unreachable endpoint is strictly better
 * than stalling the agent: telemetry never blocks the host.
 *
 ...
 * (verified during the guard-hook work: still 40s+). Every OTLP-touching
 * await on the path has to be inside the budget, and they have to SHARE
 * one deadline — N sequential 5s races would be an N×5s ceiling.
 *
 * Same backstop-timer shape as `SHUTDOWN_BACKSTOP_MS` in scanner-hook.ts
 * and `WRITE_STALL_TIMEOUT_MS` in hook-cli.ts: race the drain against an
 * unref'd timer and continue regardless of which one wins.
 */
