...
/**
 * Helpers shared by more than one ConversationSource: how much to trust
 * a thinking block, and how to read a usage record defensively.
 *
 * The thinking-fidelity decision is keyed to the MODEL. Adding support
 * for a new model family means adding one entry to `MODEL_RULES` with
 * the reason it earns its value; until then that family reports
 * `'unknown'`, which is a real answer and not a placeholder for
 * `'summary'`.
 */
...
const MODEL_RULES: readonly FidelityRule[] = [
  {
    ...
    why: "OpenAI's open-weight reasoning family; its reasoning output is a step "
      + 'narration, not a verbatim chain-of-thought contract.',
  },
  {
 ...
 * `openrouter`, `amazon-bedrock`, `azure`, `groq`, `vertex`, …) are
 * access channels and carry no fidelity information. They match nothing
 * here and correctly fall through to `unknown`.
 */
 ...
 *
 * Judged from the MODEL, because that is where the property lives.
 * Provider names the channel; the model decides what comes down it. A
 * rule keyed on the provider string mislabels every model reached
