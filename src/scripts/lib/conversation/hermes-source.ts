 ...
 *
 * Field reality, from a live capture (Hermes fronting gpt-5.5): `extra`
 * carries `user_message`, `assistant_response`, `conversation_history`,
 * `model`, `platform`.
 *
 ...
 * signal (see module doc). Where an item also carries `_issuer_kind`,
 * require it to mention "codex"; an item with no `_issuer_kind` field
 * at all still counts, since the capture this module is built from
 * never had that key populated.
 */
 ...
 * dedup.
 *
 * Preference order: `codex_reasoning_items[0].id` (a provider-side id,
 * stable because it names the item, not its position) → the first tool
 * call's id (also provider-issued and position-independent) → the
 * index-based fallback, kept only for messages that carry neither.
 */
 ...
 * replays the ENTIRE `conversation_history`. Returning one `ChatCall`
 * per historical assistant message therefore re-emits every earlier call
 * at every subsequent turn — N(N+1)/2 chat spans for N calls, each under
 * a fresh span id in a fresh trace, so no backend dedups them.
 *
 * Why 2 and not 1: a tool call is *issued* by one LLM call and
 * *executes* during the next one, so a tool span in flight at `endTurn`
 * belongs to either the current call or the one immediately before it.
 * Hermes calls are `timing: 'synthetic'`, so attribution runs purely off
 * `tool_use` ids — an id that fell out of this window would leave its
 * tool span orphaned onto the turn root.
 *
 * Why not dedup-by-id instead: the callIds available here are only
 * partly provider-issued (`stableCallId` falls back to a position-derived
 * `hermes-{i}`), so an id-only dedup drifts when history is trimmed.
 */
