...
function callFrom(id: string, acc: Accum, sinceMs: number): ChatCall | null {
  ...
  for (const p of acc.parts) {
    ...
    // `synthetic: true`, opencode's marker for text the HOST injected
    // (an interruption notice, a re-prompt, a compaction stub). A
    // ChatCall's blocks are the model's output; recording a synthetic
    // part as a `text` block is not a lossy record but a wrong one —
    // the trace would quote the harness as the assistant.
  }
