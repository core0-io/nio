    ...
    callsSince(sinceMs: number): ChatCall[] {
      ...
      for (const raw of events) {
        ...
        if (e.kind === 'message') {
          ...
          if (!id) continue;
          // message.updated is a cumulative SNAPSHOT republished on every
          // change — later envelopes replace earlier ones rather than
          // accumulating, or totals would compound.
