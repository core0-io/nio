 ...
 * `password`. That catches structured payloads but misses secrets in
 * prose: model `thinking` narrating a key it was just handed, or a
 * `tool_input.command` containing a literal
 ...
 * caught here. The `extraPatterns` parameter is the extension point for
 * them; nothing in the collector passes it yet, so today that means such
 * formats go unredacted rather than being caught by shape alone.
 */
...
const BUILTIN_PATTERNS: RegExp[] = [
  ...
  // The bare form excludes the two prefixed forms by lookahead so they
  // are not matched twice, and requires 20+ trailing chars so short
  // unrelated "sk-" substrings do not match.
