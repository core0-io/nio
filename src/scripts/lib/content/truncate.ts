 ...
 *
 * The existing `redactAndTruncate` (traces-collector.ts) caps span
 * *attribute* payloads at a flat 2 KB by `string.length` — a character
 * count. That's wrong for UTF-8 content: CJK / emoji-heavy text runs
 * ~3 bytes per character, so a character-based cap lets the wire payload
 * balloon to roughly 3x the intended byte budget. It can also slice a
 * multi-byte character in half, producing a string whose UTF-8 encoding
 * is invalid (or silently mangled via replacement characters).
 *
 ...
 *
 * Why a byte cap exists at all: content capture is meant to catch
 * *pathological* output (a `cat` of a huge file, a `find /` dump), not to
 * clip normal conversation. Typical `thinking` and replies run 2–10 KB —
 * nowhere near the defaults here. The cap exists because unbounded
 * content can violate real limits downstream: OTLP gRPC's default 4 MB
 * per-message ceiling (which fails the *entire* export, not just the
 * oversized field), the hook process blocking its host synchronously
 * while it serializes/sends a giant payload, and backend-side per-log
 * length limits.
 *
...
// Matches the suffix used by the existing character-based
// redactAndTruncate, so both code paths read the same way in exported
// content.
