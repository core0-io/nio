...
/**
 * Harness for driving the bundled Hermes `hook-cli.js` from tests.
 *
 * The point is the BUNDLE, not the harness: several defects in this area
 * are only reachable through the shipped single-file build, so a test
 * that imports the TypeScript sources cannot see them (see
 * `HERMES_HOOK_CLI` below).
 */
 ...
 * and the two bundles are built differently: the Hermes one is a ~4 MB
