 ...
 *   exports dropped by the resulting backoff pause  27
 *
 * i.e. nio opened its own circuit breaker against a healthy endpoint.
 *
...
describe('metrics flush coalescing', () => {
  it('does not stack concurrent exports against the exporter concurrency limit', async () => {
    ...
    const backoffs = lines.filter((l) => l.includes('otlp_export_backoff'));
    ...
    assert.equal(
      backoffs.length,
      0,
      'a healthy endpoint must never trip the export backoff',
    );
    assert.ok(
