 ...
 *   4. `guard-hook.ts`'s own second read of `payload.tool_name`, feeding
 *      `parseMcpToolName` — post-decision, and NOT behind `tracerProvider`
 *
 ...
 *   - revert `asText` on guard-hook.ts's `toolName`
 *     → the numeric / object / array tool_name cases go red again, at the
 *       later site (the adapter fix alone is not enough)
 */
