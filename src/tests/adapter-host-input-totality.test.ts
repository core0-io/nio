...
describe('asText', () => {
  ...
  it('is unmoved by a 200 000-deep value', () => {
    // Deliberately NOT a "degrades to ''" case. `JSON.stringify` without
    // an indent argument is iterative in V8, so no depth makes this
    // branch throw — the pretty-printing form (`JSON.stringify(x, null, 2)`,
    // which the orchestrator's mcp_tool_call branch uses) is the recursive
    // one, and that stack overflow is pinned by
    // guard-engine-error-fails-by-risk.test.ts instead. Stated here so a
    // reader does not add a depth case to the catch above and conclude it
    // verifies something.
    let deep: unknown = 'leaf';
    for (let i = 0; i < 200_000; i++) deep = { n: deep };
    assert.equal(asText(deep).slice(0, 8), '{"n":{"n');
  });
