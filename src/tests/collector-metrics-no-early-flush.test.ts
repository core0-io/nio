 ...
 *   SessionEnd       5175 ms             SessionEnd        218 ms
 *   /v1/traces POSTs: 1  (turn tree)     /v1/traces POSTs: 2
 *   state shard:      LEFT ON DISK       state shard:      discarded
 *
 * The missing second `/v1/traces` POST is the session span: `recordTurn`
 * sits between `endTurn` and `emitSessionSpan` in the SessionEnd branch,
 * so a stalled METRICS endpoint silently costs a TRACES span and leaks
 * the session's state shard and monitor-store entry — with the traces
...
describe('collector-core does not await a metrics flush inside a dispatch', () => {
   ...
   * rather than a note: everything the SessionEnd branch does AFTER
   * `recordTurn` is what a stalled metrics endpoint used to cost.
   */
  it('SessionEnd still emits the session span and discards the shard when metrics never flush', async () => {
    const logsConfig = freshLogsConfig();
    const meter = stalledMeterProvider();
    const tracer = makeInMemoryTracer();
    const sessionId = 'sess-noflush-end';

    await within('SessionStart dispatch', dispatchCollectorEvent({
      event: 'SessionStart', input: { session_id: sessionId, cwd: '/tmp' },
    ...
    assert.ok(loadState(logsConfig, sessionId), 'precondition: the session has a state shard');

    await within('SessionEnd dispatch', dispatchCollectorEvent({
      event: 'SessionEnd', input: { session_id: sessionId, cwd: '/tmp' },
      platform: 'claude-code', config: baseConfig, meterProvider: meter.provider,
      tracerProvider: tracer.provider, logsConfig,
    }));
    ...
    assert.ok(
      ...
      `the session span must still be exported; got: ${spans.map((s) => s.name).join(', ')}`,
    );
    assert.equal(
      loadState(logsConfig, sessionId), null,
      'the session shard must still be discarded at SessionEnd',
    );
  });
});
