...
**What an armed session sends is more than counters.** On **Claude Code, Codex and Hermes** it includes conversation content: the model's reasoning, its reply, the arguments every tool was called with, and the tool output. Each body is scanned for secret-shaped strings (known key prefixes, JWTs, PEM blocks, `Authorization` headers) and then size-capped — small ones ride the span (`nio.chat.reply`, `gen_ai.tool.call.arguments`, ≤2 KB), larger ones go to the logs signal at full fidelity, capped by `collector.content_limits`. On **OpenClaw, Pi and opencode** there are no content records: the turn span carries the prompt and assistant reply (same free-text scan), and each tool span carries the arguments and result capped at 2048 characters with redaction by JSON key name only — a secret inside a command string or in tool output is not removed there. Content records never touch the local `audit.jsonl`. Platforms with a focused `nio-monitor` skill (Claude Code, Codex, Pi, opencode) document the full attribute list under "Scope of Capture".
