...
**What an armed session sends is more than counters.** On **all six hosts** it includes conversation content: the model's reasoning, its reply, the arguments every tool was called with, and the tool output. Each body is scanned for secret-shaped strings (known key prefixes, JWTs, PEM blocks, `Authorization` headers) and then size-capped — small ones ride the span (`nio.chat.reply`, `gen_ai.tool.call.arguments`, ≤2 KB), larger ones go to the logs signal at full fidelity, capped by `collector.content_limits`. Two things differ on **OpenClaw, Pi and opencode**: their tool spans carry `gen_ai.tool.call.arguments` *and* `gen_ai.tool.call.result`, capped at 2048 characters and redacted by JSON key name only — so a secret inside a command string or in tool output is removed from the log record there but still rides that span attribute (no hook-driven host puts a tool result on a span at all) — and their `tool_input` / `tool_output` records are emitted unconditionally rather than only on overflow, so a small tool body is sent twice. Reasoning is host-dependent: Pi and opencode publish it, OpenClaw does not. Content records never touch the local `audit.jsonl`. Platforms with a focused `nio-monitor` skill (Claude Code, Codex, Pi, opencode) document the full attribute list under "Scope of Capture".
